...
#include "HDCTBasis2.h"
#include "healpixparam.h"

HDCTBasis2::HDCTBasis2( int basis_num, int n_side )
{
  if( basis_num%12 != 0 )
  {
    printf( "[Error] HDCTBasis2, number of basis must be divisable by 12\n" );
    exit(-1);
  }
  ...
  zz2ct = (int*) malloc( N_SIDE*N_SIDE*sizeof(int) );
  gen_zmap( zz2ct, N_SIDE );
...
GParam* HDCTBasis2::GetParam( int param_density )
{
...
HDCTBasis2::~HDCTBasis2()
{
  SAFE_FREE( zz2ct );
}

void HDCTBasis2::GetSamples( float *pSample, const FLOAT3 *tL ) const
{
  ...
  float *tSample = &pSample[face_num];

  for( int zig_t=0; zig_t<N_DCT; zig_t++, tSample+=12 )
  {
    zx = zz2ct[zig_t]%N_SIDE;
    zy = zz2ct[zig_t]/N_SIDE;
...
float HDCTBasis2::mean( const float *c0 )
{
  float mean_val = 0;
...
float HDCTBasis2::hp_val( const float *c0, const int ipix )
{
  float res = 0;
...
float HDCTBasis2::hp_val( const float *c0, int face_num, float x, float y )
{
  float res = 0;
