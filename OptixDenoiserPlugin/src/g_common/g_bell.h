...
//   format <- {GL_RGB, GL_ALPHA, GL_RGBA}
//   default format = GL_RGB
void save_fbo( const char *spath, void (*display_func)(), GLenum format=0x1907, int width=0, int height=0 );
void save_fbo( GPfm &buf_xyz, GPf1 &buf_w, void (*display_func)(), int width=0, int height=0 );
void save_fbo( GPfm &buf_xyz, void (*display_func)(), int width=0, int height=0 );
...
class GBell
{
  public:
    ...
    void update_sxy(int x, int y, double ss);
    void update_sxy(int x, int y);
