...
void GPfm::save( const char *spath, const char *_mode ) const
{
  ...
  if( strcmp( mode, "pfm" )==0)
  {
    FILE *f0 = fopen( spath, "wb" );
      if(f0==NULL)
      {
        printf( "[Error] : GPfm::save(), cannot open output file %s\n", spath );
        exit(0);
      }
      fprintf( f0, "PF\n");
	    fprintf( f0, "%i %i\n", w, h );
...
float GPfm::vmean1() const
{
  ...
  res = 0;
  for( t=0; t<l; t++, tm++ )
    res = res + (tm->x+tm->y+tm->z)/3;

  return res/l;
}

float GPfm::variance1() const
{
  int t, l;
  FLOAT3 *tm;
  float mn;
...
void GBmp::save( const char *spath ) const
{
  ...
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GBmp::save(), cannot open output file %s\n", spath );
      exit(0);
    }
...
void GPPm::save( const char *spath ) const
{
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GPPm::save(), cannot open output file %s\n", spath );
      exit(0);
    }
    fprintf( f0, "P6\n");
	  fprintf( f0, "%i %i\n", w, h );
...
void GPf4::save( const char *spath ) const
{
  ...
  if( strcmp( gp.ename, "pf4" )==0 )
  {
    ...
    FILE *f0 = fopen( spath, "wb" );
      if(f0==NULL)
      {
        printf( "[Error] : GPf4::save(), cannot open output file %s\n", spath );
        exit(0);
      }
      fprintf( f0, "pf\n");
	    fprintf( f0, "%i %i\n", w, h );
...
void GPf4::add( const GPf4 &A )
{
  if( w!=A.w || h!=A.h )
  {
    printf( "[Error] : GPf4::add(), the pf4 are not of the same dimension.\n" );
    exit(-1);
  }
  int t, l;
  float *tn, *sn;
...
FLOAT4 GPf4::mse( const GPf4 &pfm0, const GPf4 &pfm1 )
{

  if( pfm0.w != pfm1.w || pfm0.h != pfm1.h )
  {
    printf( "[Error] GPf4::mse(), images is not of the same dimension.\n" );
    exit(-1);
  }
...
void GPf1::save( const char *spath ) const
{
  ...
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GPf1::save(), cannot open output file %s\n", spath );
      exit(0);
    }
    fprintf( f0, "Pf\n" );
	  fprintf( f0, "%i %i\n", w, h );
...
void GPng::save( const char *spath ) const
{
  ...
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GPng::save(), cannot open output file %s\n", spath );
      exit(0);
    }
...
void GJpg::save( const char *spath ) const
{
  ...
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GJpg::save(), cannot open output file %s\n", spath );
      exit(0);
    }
    cinfo.err = jpeg_std_error(&jerr);
...
void GPim::save( const char *spath ) const
{
  FILE *f0 = fopen( spath, "wb" );
    if(f0==NULL)
    {
      printf( "[Error] : GPim::save(), cannot open output file %s\n", spath );
      exit(0);
    }
    fprintf( f0, "PI\n" );
    fprintf( f0, "%i %i %i\n", w, h, n_code );
