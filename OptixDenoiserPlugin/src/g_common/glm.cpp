...
glmReadDirectXM( char* filename )
{
  ...
  FILE* file = fopen(filename, "rb");
    if( !file )
    {
      fprintf(stderr, "glmReadDirectXM() failed: can't open data file \"%s\".\n", filename);
      exit(1);
    }
  fclose(file);
    ...
    model->numvertices  = n_vertex;
    model->numnormals   = n_vertex;
    model->numtriangles = n_face;

    model->vertices  = (GLfloat*)    malloc( (model->numvertices + 1) * 3 * sizeof(GLfloat) );
    model->normals   = (GLfloat*)    malloc( (model->numnormals  + 1) * 3 * sizeof(GLfloat) );
    model->triangles = (GLMtriangle*)malloc(  model->numtriangles         * sizeof(GLMtriangle) );
    memset( model->triangles, 0, model->numtriangles * sizeof(GLMtriangle) );

    GLMgroup* group = glmAddGroup(model, "default");
      group->numtriangles = n_face;
      group->triangles = (GLuint*)malloc(sizeof(GLuint) * group->numtriangles);
    ...
    FILE *f1 = fopen( filename, "rb" );
      while( fgets( str, 256, f1 ) )
      {
        ...
        if( strcmp( "Vertex", tag0 )==0 )
        {
          ...
          sscanf( str, "Vertex %i %f %f %f {normal=(%f %f %f)}",
            &v_idx,  &vx, &vy, &vz,   &nx, &ny, &nz );

          model->vertices[3*v_idx+0] = vx;
          model->vertices[3*v_idx+1] = vy;
          model->vertices[3*v_idx+2] = vz;
          
          model->normals[3*v_idx+0] = nx;
          model->normals[3*v_idx+1] = ny;
          model->normals[3*v_idx+2] = nz;
        }
        ...
        if( strcmp( "Face", tag0 )==0 )
        {
          ...
          int v0,v1,v2;
          //Face 1  2165 111 526
          sscanf( str, "Face %i %i %i %i", &f_idx, &v0, &v1, &v2 );
          f_idx -= 1;

          model->triangles[f_idx].vindices[0] = v0;
          model->triangles[f_idx].vindices[1] = v1;
          model->triangles[f_idx].vindices[2] = v2;

          model->triangles[f_idx].nindices[0] = v0;
          model->triangles[f_idx].nindices[1] = v1;
          model->triangles[f_idx].nindices[2] = v2;

          group->triangles[f_idx] = f_idx;
        }
      }
...
/* glmTranslate: Translate a model by a given amount.
 * 
 * model - properly initialized GLMmodel structure
 * move by - (x,y,z)
 */
...
GLvoid glmMarkGroupMode( GLMmodel* model )
{
  GLMgroup* group;
    group = model->groups;
  ...
  GLMtriangle* triangle;
    ...
    while( group ) 
    {
      ...
      if( group->material )
      {
        group->mode = group->mode | GLM_MATERIAL;
        group->mode = group->mode | GLM_COLOR;
      }
        
      bool flag_glm_flat    = true;
      bool flag_glm_smooth  = true;
      bool flag_glm_texture = true;

      for (i = 0; i < group->numtriangles; i++) 
      {
        triangle = &T(group->triangles[i]);

        if( triangle->findex==0 )
        {
          flag_glm_flat = false;
          break;
        }
      ...
      for (i = 0; i < group->numtriangles; i++) 
      {
        ...
        if( 
          triangle->nindices[0]==0 ||
          triangle->nindices[1]==0 ||
          triangle->nindices[2]==0 
        ){
          flag_glm_smooth = false;
          break;
        }
      }
      ...
      for (i = 0; i < group->numtriangles; i++) 
      {
        ...
        if( 
          triangle->tindices[0]==0 ||
          triangle->tindices[1]==0 ||
          triangle->tindices[2]==0 
        ){
          flag_glm_texture = false;
          break;
        }
      }

      if( flag_glm_flat )
        group->mode = group->mode | GLM_FLAT;
      if( flag_glm_smooth )
        group->mode = group->mode | GLM_SMOOTH;
      if( flag_glm_texture )
        group->mode = group->mode | GLM_TEXTURE;
...
/*    
    group = model->groups;
    while (group) {
        GLuint grp_mode = group->mode;

        if(
          (mode & GLM_TEXTURE) == (group->mode & GLM_TEXTURE)
        ){
        ...
        for (i = 0; i < group->numtriangles; i++) {
            ...
            if( mode & GLM_FLAT && mode & grp_mode )
                glNormal3fv(&model->facetnorms[3 * triangle->findex]);
            
            if (mode & GLM_SMOOTH && mode & grp_mode )
                glNormal3fv(&model->normals[3 * triangle->nindices[0]]);

            if (mode & GLM_TEXTURE && mode & grp_mode )
