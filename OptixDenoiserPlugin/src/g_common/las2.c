...
//   char *in1, *in2, *out1, *out2;
   ...
   int l,ll;
    ...
    valueb=(double*)malloc(sizeof(double)*ncol*ncol);

    size1 = sizeof(double) * (6 * n + nrow + nnzero + (n * lanmax));
    if (!(value  = (double *) malloc(size1)) || !(valueb)){
	perror("MALLOC FAILED in MAIN()");
	exit(errno);
    }
    tptr1 = value;
    /* allocated memory including work array used in landr */
    memory = size1 + sizeof(double) * (5 * n + lanmax * 4 + 1 + ncol*ncol);
    ...
    // Calculate B = A^T * A
    for (i=0,l=0; i<ncol; i++)
      for (k=0; k<ncol; k++,l++)
        for (ll=0; ll<nrow; ll++)
          valueb[l]+=(value[ll*ncol+i]*value[ll*ncol+k]);
    ...
    if (vectors) {
	  ...
	  //write(fp_out2, (char *)&xv1[ida], size1);
          ...
          if (i>=nsig-SVDK)
	    for (k=0; k<nrow; k++)
	      valueu[nsig-i-1+k*SVDK] = (float)xv1[ida+k];
	  ...
	  id += nn;
       ...
       #endif
       for (i=0; i<SVDK; i++)
         values[SVDK-i-1] = (float)d[i+nsig-SVDK];
    }
    else {
       for (i = j; i >= 0; i--)
...

{
 ...
 fprintf(fp_out1, " %s\n", title);
 fprintf(fp_out1, "           %s\n", name);
 fprintf(fp_out1, " ... NO. OF DOCUMENTS (ROWS)   = %8ld\n", nrow);
 fprintf(fp_out1, " ... NO. OF TERMS     (COLS)   = %8ld\n", ncol);
...

{
   ...
   for (k=0,tmp=(nsig-SVDK)*(n+nrow); k<SVDK; k++,tmp+=nrow)
     for (i=0; i<n; i++,tmp++)
       valuev[SVDK-k-1+i*SVDK] = (float)xv1[tmp];
   free(s);
   return;
}
...

{
   ...
   for (id = 0; id < 3; id++) {
      if (id > 0 || j > 0 || rnm2 == 0) 
	 for (i = 0; i < n; i++) r[i] = myrandom(&irand);
 ...
 ***********************************************************************/
static double myrandom(long *iy)
