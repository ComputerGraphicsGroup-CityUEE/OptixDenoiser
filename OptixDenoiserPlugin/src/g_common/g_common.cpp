...
wGPath wparse_spath( const wchar_t *spath0 )
{
  wGPath gp;
  int i;
  int slen;
  ...
  slen = wcslen(spath0);  
  spath = (wchar_t*) malloc( (slen+1)*sizeof(wchar_t) );
  wcscpy( spath, spath0 );
  ...
  const wchar_t *t_fname = wcsrchr( spath, L'/' );

  int dlen, flen;
  if( t_fname )
  {
    dlen = int( slen - wcslen(t_fname) +1 );
    t_fname++;
  }else
  {
    dlen = 0;
  ...
  memcpy( gp.dname, spath, dlen*sizeof(wchar_t) );
  gp.dname[dlen] = L'\0';
...
int printf( const wchar_t *format, ... )
{
  ...
  va_start(args, format);
    ...
    wchar_t *buf = (wchar_t*) malloc( len*sizeof(wchar_t)  );
      ...
      fflush(stdout);
      int prev = _setmode(_fileno(stdout), _O_U8TEXT);
        wprintf( buf );
      fflush(stdout);
...
int sprintf( wchar_t *buf, const char *format, ... )
{
  wchar_t *wformat = (wchar_t*) malloc( (strlen(format)+1)*sizeof(wchar_t) );
  mbstowcs( wformat, format, strlen(format) );
  va_list args;
  va_start(args, format);
    vswprintf(buf, 512, wformat, args);
  va_end(args);
  free(wformat);
  return 0;
}
